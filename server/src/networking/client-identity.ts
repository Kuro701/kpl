	...
	return {
		...
		async requestAuth() {
			...
			if (data.provider === 'anonymous') {
				...
				const [googleUser, error] = await safeAwait(getGoogleProfile(data.user_token));
				...
				const name = googleUser.name || data.username;
				const playerUUID = `google_${googleUser.id}`;
				const image = googleUser.picture || `https://cdn.discordapp.com/embed/avatars/0.png`;
				player = createPlayer(name, playerUUID, image, networkKit);
